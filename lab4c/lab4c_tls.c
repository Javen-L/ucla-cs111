...
void write_log(float temperature){
    ...
    if (com[3]){
        ...
        retval = SSL_write(ssl, write_format, strlen(write_format));
        if (retval <= 0){
            error_msg("SSL_write");
            exit(2);
        }
...
int create_socket(int port_num){
    ...
    if (ctx == NULL){
        fprintf(stderr, "error: failed to create SSL_CTX object\n");
        exit(2);
    }
    ssl = SSL_new(ctx);
...
void *server_thread(void *com_){
    ...
    while(com[3]){
        ...
        if (poll_fd.revents & POLLIN){
            ...
            read_size = SSL_read(ssl, buffer, BUF_SIZE);
	        if (read_size < 0){
                error_msg("read");
                exit(2);
            }
